...
**Graph-first.** WTFcode reads your repo once and builds a persistent knowledge graph of every concept, relationship, and design decision it finds. When you ask a question, it navigates the graph instead of scanning files — so you get a structured answer, not a grep dump. The graph survives across sessions; you don't pay to re-read the codebase every time.

**Three-pass extraction.** Code files are parsed deterministically with tree-sitter (no AI needed, no hallucination). Video and audio are transcribed locally with Whisper. Everything else — docs, papers, images — goes through Claude subagents running in parallel. Each pass feeds into one graph, so a comment in a Python file and a paragraph in a PDF can end up as neighbors if they describe the same idea.

**Token discipline.** The graph is the compression layer. Instead of pasting 50 files into a prompt, you query a compact representation of what those files contain. On a real mixed corpus this produces 71.5x fewer tokens per query. The same discipline is encoded in the assistant rules: prefer graph summary, use focused follow-ups, avoid broad scans when a path query can answer.
...
The token minimization rules in `CLAUDE.md` and the 71.5x reduction benchmark are two independent observations of the same behavior. The rules say: prefer graph summary over full file reads, answer from structure rather than raw text, use focused follow-up queries instead of broad context loads. The benchmark measures what happens when you follow those rules across a real corpus: 71.5x fewer tokens per query compared to reading raw files. This is not a coincidence — context discipline and output constraints are the primary token-cost lever, and the benchmark confirms the rules work.
